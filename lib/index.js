...
const server = net.createServer({
  ...
  socket.on('end', () => {
    switch (data.trim().toLowerCase()) {
      case 'right': return socket.end(statusRight + '\n');
...
const getWhoamiAndHost = co.wrap(function * getWhoamiAndHost() {
  const { stdout: whoami } = yield execa('whoami');
  const { stdout: hostname } = yield execa('hostname');

  return colorWhoami(whoami) + '@' + colorHostname(hostname);
});

let statusRight = '';
const updateStatusRight = co.wrap(function * updateStatusRight() {
  const temperature = yield getTemperature();
  const cpu = yield getCPU();
  const ram = yield getRAM();
  const clock = yield getClock();
  const whoamiAndHost = yield getWhoamiAndHost();
  let battery = '';

  if (hasBattery) {
    battery = yield getBattery();
  }

  let res = ' ';
  res += whoamiAndHost + ' ' + COLUMN;
  ...
  statusRight = colorMainBG(colorMainFG(res));
});
